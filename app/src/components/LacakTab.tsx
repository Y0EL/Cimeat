...
import { ChefHat, Flame, RefreshCcw, Share2, Sparkles, Trophy, Zap } from "lucide-react";
import { useState } from "react";
import { MacroBar, WeeklyChart } from "./Common";
import ShareGenerator from "./ShareGenerator";

export default function LacakTab({ settings, dailyMacros, streak, weeklyData, weekMac, aiRecommendation, loadingRec, onRefreshRec, onOpenRecipe, history }: any) {
   ...
   return (
      <motion.div key="lacak" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6 pb-36">
         ...
         <ShareGenerator
            ...
            onComplete={() => setTriggerShare(false)}
            {...{ history, weeklyData, streak, settings, dailyMacros, aiRecommendation }} 
         />
         ...
         {/* Share Achievement Hero */}
         <motion.div
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            onClick={() => setTriggerShare(true)}
            className="bg-gradient-to-r from-orange to-[#FF8C61] p-6 rounded-[2.5rem] text-white flex items-center justify-between shadow-xl shadow-orange/20 border border-white/10 cursor-pointer overflow-hidden relative group"
         >
            ...
            <div className="flex items-center gap-4 relative z-10">
               ...
               <div>
                  <h4 className="font-black text-lg tracking-tight italic uppercase">BAGIKAN</h4>
                  <p className="text-[10px] font-black uppercase tracking-widest opacity-80 decoration-white/20 underline underline-offset-4">Kasih tau temen!</p>
               </div>
            </div>
